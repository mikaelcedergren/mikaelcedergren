...
Mikael Cedergren's personal portfolio, rebuilt as an **Angular 22 SSG app** — prerendered to
...
pnpm build              # generate sitemap, prerender pages, then compile the server
pnpm build:release      # browser-only staged prerendering used by the shared release command
...
- **Sections** at `<dir>/index.html`: `/about/`, `/concepts/`, `/resume/`.
- **Blog** links to Substack; blog content is not hosted in this repository.
- Home at `/`.
